...
/*
  ...
  Find minimum element and swap to front, repeat.
...
interface Step {
  line: number;
  action: string;
  type: "init" | "new-pass" | "compare" | "new-min" | "swap" | "no-swap" | "complete";
  array: number[];
  i: number;
  j: number;
  minIndex: number;
...
function computeAllSteps(inputArray: number[]): Step[] {
  ...
  allSteps.push({
    line: 1,
    action: `Initialize selection sort with array of ${n} elements`,
    type: "init",
    array: [...array],
    i: 0,
    j: 0,
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    allSteps.push({
      line: 3,
      action: `Pass ${i + 1}: Find minimum in unsorted portion [${i}..${n - 1}]`,
      type: "new-pass",
      array: [...array],
      i,
      j: i,
      ...
      swapping: null,
      sorted: new Set(sorted),
      currentMin: array[minIndex],
    });

    // Assume first element is minimum
    allSteps.push({
      line: 4,
      action: `Assume minimum is array[${i}] = ${array[i]}`,
      type: "new-min",
      array: [...array],
      i,
      j: i,
    ...
    for (let j = i + 1; j < n; j++) {
      ...
      allSteps.push({
        line: 5,
        action: `Compare: array[${j}] = ${array[j]} vs min = ${array[minIndex]}`,
        type: "compare",
        array: [...array],
        i,
        j,
      ...
      if (array[j] < array[minIndex]) {
        ...
        allSteps.push({
          line: 6,
          action: `New minimum found! ${array[j]} < ${array[oldMin]}, update minIndex = ${j}`,
          type: "new-min",
          array: [...array],
          i,
          j,
        ...
        allSteps.push({
          line: 5,
          action: `${array[j]} ≥ ${array[minIndex]}, keep current minimum`,
          type: "compare",
          array: [...array],
          i,
          j,
    ...
    if (minIndex !== i) {
      allSteps.push({
        line: 7,
        action: `Swap: array[${i}] = ${array[i]} ↔ array[${minIndex}] = ${array[minIndex]}`,
        type: "swap",
        array: [...array],
        i,
        j: n - 1,
      ...
      allSteps.push({
        line: 7,
        action: `After swap: array[${i}] = ${array[i]}`,
        type: "swap",
        array: [...array],
        i,
        j: n - 1,
      ...
      allSteps.push({
        line: 7,
        action: `No swap needed: minimum ${array[i]} already at position ${i}`,
        type: "no-swap",
        array: [...array],
        i,
        j: n - 1,
    ...
    allSteps.push({
      line: 8,
      action: `Position ${i} is now sorted with value ${array[i]}`,
      type: "complete",
      array: [...array],
      i,
      j: n - 1,
  ...
  allSteps.push({
    line: 9,
    action: "Selection sort complete! Array is fully sorted.",
    type: "complete",
    array: [...array],
    i: n - 1,
    j: n - 1,
...
const algorithmCode = [
  { line: 1, content: "selectionSort(A, n)", tokens: [
    { type: "function", text: "selectionSort" },
    { type: "bracket", text: "(" },
  ...
  { line: 4, content: "      minIdx ← i", tokens: [
    { type: "plain", text: "      " },
    { type: "variable", text: "minIdx" },
    { type: "operator", text: " ← " },
    { type: "variable", text: "i" },
  ]},
  { line: 5, content: "      for j ← i+1 to n-1 do", tokens: [
    { type: "plain", text: "      " },
    { type: "keyword", text: "for" },
    { type: "plain", text: " " },
  ...
  { line: 6, content: "         if A[j] < A[minIdx] then", tokens: [
    { type: "plain", text: "         " },
  ...
  { line: 7, content: "            minIdx ← j", tokens: [
    { type: "plain", text: "            " },
    { type: "variable", text: "minIdx" },
    { type: "operator", text: " ← " },
    { type: "variable", text: "j" },
  ]},
  { line: 8, content: "      swap A[i] ↔ A[minIdx]", tokens: [
    { type: "plain", text: "      " },
...
export default function SelectionSortPage() {
  ...
  const getBarColor = (index: number) => {
    ...
    // Current minimum
    if (currentStep.minIndex === index && currentStep.type !== "init") {
    ...
    // Current i position (start of unsorted)
    if (currentStep.i === index) {
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col font-mono">
      ...
      <header className="bg-[var(--bg-secondary)] border-b border-[var(--border-color)] px-4 py-3 flex items-center justify-between flex-wrap gap-4">
        ...
        <Link href="/algorithms" className="flex items-center gap-2 group">
          ...
          <span className="text-[var(--text-primary)]">selectionsort</span>
        </Link>
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 min-w-[400px]">
          ...
          <Section title="array visualization" className="flex-1">
            <div className="relative bg-[var(--bg-primary)] border border-[var(--border-color)] rounded overflow-hidden p-4" style={{ minHeight: 320 }}>
              {/* Current minimum indicator */}
              {currentStep && currentStep.currentMin !== null && (
                <div className="absolute top-4 right-4 flex items-center gap-2 bg-[var(--bg-secondary)] px-3 py-2 rounded border border-[var(--accent-red)]">
                  <span className="text-xs text-[var(--text-muted)]">Current min:</span>
                  <span className="text-lg font-bold text-[var(--accent-red)]">{currentStep.currentMin}</span>
                  <span className="text-xs text-[var(--text-muted)]">at index {currentStep.minIndex}</span>
                </div>
              )}
                ...
                <div className="flex items-end justify-center gap-1 mt-2">
                  {displayArray.map((_, index) => (
                    <div
                      ...
                      style={{ width: Math.max(30, 400 / displayArray.length) }}
                    >
                      {currentStep.i === index && !currentStep.sorted.has(index) && (
                        <span className="text-[var(--accent-orange)]">i</span>
                      )}
                      {currentStep.j === index && currentStep.type === "compare" && (
                        <span className="text-[var(--accent-yellow)]">j</span>
                      )}
                      {currentStep.minIndex === index && currentStep.type !== "init" && !currentStep.sorted.has(index) && (
                        <span className="text-[var(--accent-red)]">min</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="absolute bottom-3 left-3 flex flex-wrap gap-3 text-xs text-[var(--text-muted)] bg-[var(--bg-secondary)]/90 px-3 py-2 rounded">
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-[var(--accent-orange)]"></span> position i
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-[var(--accent-yellow)]"></span> comparing j
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-3 h-3 bg-[var(--accent-red)]"></span> current min
                </span>
                <span className="flex items-center gap-1">
            ...
            <Section title="variables">
              <div className="grid grid-cols-4 gap-2">
                <VariableCard
                  name="i"
                  value={currentStep?.i ?? "—"}
                  active={currentStep?.type === "new-pass"}
                />
                <VariableCard
                  name="j"
                  value={currentStep?.j ?? "—"}
                  active={currentStep?.type === "compare"}
                />
                <VariableCard
                  name="minIdx"
                  value={currentStep?.minIndex ?? "—"}
                  active={currentStep?.type === "new-min"}
                />
                <VariableCard
                  name="A[min]"
                  value={currentStep?.currentMin ?? "—"}
                  active={currentStep?.type === "new-min"}
                />
              </div>
            </Section>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-[var(--bg-primary)] border border-[var(--border-color)] p-2 text-center">
                  ...
                  <div className="text-lg text-[var(--accent-orange)]">
                    {currentStep ? currentStep.i + 1 : 0} / {displayArray.length - 1}
                  </div>
                </div>
              </div>
            </Section>
            ...
            <div className="flex items-center justify-center gap-8 min-h-[60px]">
              {currentStep?.type === "compare" && currentStep.comparing ? (
                <>
                  ...
                  <div className="text-center">
                    <div className="text-xs text-[var(--text-muted)] mb-1">A[minIdx] = A[{currentStep.minIndex}]</div>
              ...
              ) : currentStep?.type === "new-min" ? (
                <div className="text-[var(--accent-red)] text-lg">
                  New minimum: {currentStep.currentMin} at index {currentStep.minIndex}
                </div>
              ) : currentStep?.type === "swap" && currentStep.swapping ? (
                <div className="text-[var(--accent-magenta)] text-lg">
                  Swapping {currentStep.array[currentStep.swapping[0]]} ↔ {currentStep.array[currentStep.swapping[1]]}
                </div>
              ) : (
                  ...
                  <span
                    className={`px-1 ${
                      currentStep?.sorted.has(idx)
                        ? "text-[var(--accent-green)]"
                        : currentStep?.minIndex === idx
                        ? "text-[var(--accent-red)]"
                        : currentStep?.comparing && currentStep.comparing[0] === idx
